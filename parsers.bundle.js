var NyoraParsers = (() => {
  ...
  var MadaraParser = class extends BaseParser {
    ...
    async getAsuraListPage(page, order, filter) {
      ...
      const html = await this.context.httpGet(url, this, { "User-Agent": DESKTOP_UA });
    ...
    async getAsuraPages(chapter) {
      ...
      if (key && number) {
        try {
          const data = JSON.parse(await this.context.httpGet(`${this.asuraApiBase()}/api/series/${key}/chapters/${number}`, this, { "User-Agent": DESKTOP_UA }));
  ...
  // src/webtoon.js
  var WebtoonParser = class extends BaseParser {
    constructor(context, source, domain, pageSize = 24) {
      super(context, source, domain, pageSize);
      this.lang = source && source.locale ? String(source.locale).split("-")[0] : "en";
      this.maxChapterPages = 300;
    }
    base() {
      return `https://${this.domain}`;
    }
    ...
    async getListPage(page, order, filter) {
      ...
      if (page > 1)
        return [];
      const url = query ? `${this.base()}/${this.lang}/search?keyword=${encodeURIComponent(query)}` : `${this.base()}/${this.lang}/originals`;
    ...
    parseCards(doc) {
      ...
      for (const a of anchors) {
        const href = a.getAttribute("href") || "";
        if (!/title_no=\d+/.test(href) && !/titleNo=\d+/.test(href))
          continue;
        const url = this.toRelativeUrl(href.split("#")[0]);
        if (seen.has(url))
          continue;
        seen.add(url);
        const title = this.text(a.querySelector(".title, .subj, h1.subj, h3.subj, strong.title")) || a.querySelector("img") && a.querySelector("img").getAttribute("alt") || "";
        if (!title)
          continue;
    ...
    async getDetails(manga) {
      ...
      let authors = authorsText ? authorsText.replace(/author|illustrator|adapted by|original work/gi, "").split(/[,/]/).map((s) => s.trim()).filter(Boolean) : [];
      ...
      const state = /COMPLETED|FINISH|END|TERMINÉ/i.test(status) ? MangaState.FINISHED : /HIATUS|REST/i.test(status) ? MangaState.PAUSED : MangaState.ONGOING;
    ...
    async collectChapters(mangaUrl) {
      ...
      for (let page = 1; page <= this.maxChapterPages; page++) {
        ...
        let html;
        try {
          html = await this.context.httpGet(pageUrl, this, this.reqHeaders());
        } catch {
          break;
        }
        const doc = this.context.parseHTML(html);
        ...
        for (const li of items) {
          ...
          if (!/viewer/.test(href))
            continue;
          const no = parseInt(li.getAttribute("data-episode-no") || (href.match(/episode_no=(\d+)/i) || [])[1] || "0", 10);
          if (!no || byNo.has(no))
            continue;
  ...
  // src/lectormoe.js
  var LectorMoeParser = class extends BaseParser {
    constructor(context, source, domain, pageSize = 36) {
      super(context, source, domain, pageSize);
    ...
    async getListPage(page, order, filter) {
      ...
      return items.map((it) => {
        ...
        return new Manga({
          id: slug,
          url: slug,
          // Internal identifier for details lookup
    ...
    async getDetails(manga) {
      ...
      const chapters = rawChapters.filter((it) => !it.isUnreleased).map((it) => {
        ...
        return new MangaChapter({
          id: chapterUrl,
          url: chapterUrl,
          title: `Cap\xEDtulo ${displayNum} - ${it.title || ""}`,
          number,
          volume: 0,
          branch: null,
          scanlator: null,
  ...
  // src/gigaviewer.js
  var GigaViewerParser = class extends BaseParser {
    constructor(context, source, domain, pageSize = 30) {
      super(context, source, domain, pageSize);
      const ov = source.overrides || {};
      this.popularMangaSelector = ov.popularMangaSelector || "ul.series-list li a";
      this.latestUpdatesSelector = ov.latestUpdatesSelector || null;
    ...
    get dayOfWeek() {
      return new Intl.DateTimeFormat("en-US", { weekday: "long", timeZone: this.timeZone }).format(/* @__PURE__ */ new Date()).toLowerCase();
    }
    getHeaders() {
      return {
        "Origin": `https://${this.domain}`,
    ...
    async getListPage(page, order, filter) {
      ...
      try {
        ...
        if (isSearch && String(e).includes("404"))
          return [];
        throw e;
      }
      const doc = this.context.parseHTML(html);
    ...
    async getDetails(manga) {
      ...
      const coverUrl = info?.querySelector("div.series-header-image-wrapper img")?.getAttribute("data-src") || info?.querySelector("div.series-header-image-wrapper img")?.getAttribute("src");
      const aggregateId = doc.querySelector("script.js-valve")?.getAttribute("data-giga_series") || doc.querySelector(".js-readable-products-pagination")?.getAttribute("data-aggregate-id");
      if (!aggregateId)
        throw new Error("Could not find series aggregate ID");
      const chapters = [];
    ...
    async getPages(chapter) {
      ...
      return pageStructure.pages.filter((p) => p.type === "main" && p.src).map((p, i) => {
        let imageUrl = p.src;
        if (isScrambled) {
  ...
  // src/comiciviewer.js
  var ComiciViewerParser = class extends BaseParser {
    constructor(context, source, domain, pageSize = 24) {
      super(context, source, domain, pageSize);
    ...
    async getListPage(page, order, filter) {
      if (filter && filter.query) {
        if (this.isAlt) {
          const url2 = `${this.apiUrl}/search?q=${encodeURIComponent(filter.query)}&page=${page}&size=${this.pageSize}`;
          const json = JSON.parse(await this.context.httpGet(url2, this));
          return this.parseSearchApi(json);
        } else {
          const url2 = `https://${this.domain}/search?keyword=${encodeURIComponent(filter.query)}&page=${page - 1}&filter=series`;
          const html2 = await this.context.httpGet(url2, this);
          return this.parseSearchHtml(html2);
        }
      }
      let url = "";
      ...
      if (this.isAlt) {
        ...
        for (const ep of orderedEpisodes) {
          ...
          chapters.push(new MangaChapter({
            ...
            url: epUrl,
            title,
            uploadDate: ep.datePublished ? ep.datePublished * 1e3 : 0,
            source: this.source,
            index: index++
          }));
        }
        ...
        for (const it of epItems) {
          ...
          if (time && time.getAttribute("datetime")) {
            ...
            try {
              const parsed = (/* @__PURE__ */ new Date(dt.replace(" ", "T") + "Z")).getTime();
              if (!Number.isNaN(parsed)) {
                uploadDate = parsed;
              }
            } catch (e) {
    ...
    async getPages(chapter) {
      ...
      if (this.isAlt) {
        ...
        if (!viewerContent)
          throw new Error("Viewer not found for this chapter");
        ...
        if (!viewer)
          throw new Error("You need to log in via WebView to read this chapter or purchase this chapter.");
  ...
  var sources_default = [
    ...
    {
      id: "ALPHACENTAURI",
      className: "AlphaCentauri",
      title: "Alpha Centauri",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "alphacentauri",
        basePath: "/alphacentauri"
      }
    },
    {
      id: "AMEFANSUB",
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "ame",
        basePath: "/ame"
      }
    },
    {
      id: "ANIMUNOMANSEBIA",
      className: "AnimuNoMansebia",
      title: "Animu No Mansebia",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "animunm",
        basePath: "/animunm"
      }
    },
    {
      id: "ANTIPATREONSCAN",
      className: "AntiPatreonScan",
      title: "AntiPatreon Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "antipatreon",
        basePath: "/antipatreon"
      }
    },
    {
      id: "ARIGATOSA",
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "arigatosa",
        basePath: "/arigatosa"
      }
    },
    {
      id: "ASPERUASTRADUS",
      className: "AsperuasTradus",
      title: "Asperuas Tradus",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "asperuastradus",
        basePath: "/asperuastradus"
      }
    },
    {
      id: "ASUKALANGLEYTRANSLATIONS",
      className: "AsukaLangleyTranslations",
      title: "Asuka Langley Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "asukatranslations",
        basePath: "/asukatranslations"
      }
    },
    {
      id: "AZUREBLOOM",
      className: "AzureBloom",
      title: "Azure Bloom",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "azurebloom",
        basePath: "/azurebloom"
      }
    },
    {
      id: "BAKAGUYASCANLATION",
      className: "BakaguyaScanlation",
      title: "Bakaguya Scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "bakaguya",
        basePath: "/bakaguya"
      }
    },
    {
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "beeggimp",
        basePath: "/beeggimp"
      }
    },
    {
      id: "BEYONDBIRTHDAYSUBS",
      className: "BeyondBirthdaySubs",
      title: "Beyond Birthday Subs",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "beyondbirthdaysubs",
        basePath: "/beyondbirthdaysubs"
      }
    },
    {
      id: "BONBONSCANLATION",
      className: "BonbonScanlation",
      title: "Bonbon Scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "bonbonscanlation",
        basePath: "/bonbonscanlation"
      }
    },
    {
      id: "BOREDSLAYERSCAN",
      className: "BoredSlayerScan",
      title: "Bored Slayer Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "boredslayerscan",
        basePath: "/boredslayerscan"
      }
    },
    {
      id: "BREAKSCAN",
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "breakscan",
        basePath: "/breakscan"
      }
    },
    {
      id: "CATHALTRADUCCIONES",
      className: "CathalTraducciones",
      title: "CathalTraducciones",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "cathaltl",
        basePath: "/cathaltl"
      }
    },
    {
      id: "CHIBISSCAN",
      className: "ChibisScan",
      title: "Chibis Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "chibis",
        basePath: "/chibis"
      }
    },
    {
      id: "CHUNCHUNMARUTRANSLATIONS",
      className: "ChunchunmaruTranslations",
      title: "Chunchunmaru Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "chunchunmarutl",
        basePath: "/chunchunmarutl"
      }
    },
    {
      id: "CRIMSONSCAN",
      className: "CrimsonScan",
      title: "Crimson Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "crimsonscan",
        basePath: "/crimsonscan"
      }
    },
    {
      id: "CULTOASLANESH",
      className: "CultoASlanesh",
      title: "Culto a Slanesh",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "cultoaslanesh",
        basePath: "/cultoaslanesh"
      }
    },
    {
      id: "DAISUKINAFANSUB",
      className: "DaisukiNaFansub",
      title: "Daisuki na Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "dnfansub",
        basePath: "/dnfansub"
      }
    },
    {
      id: "DALETHPROJECT",
      className: "DalethProject",
      title: "Daleth Project",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "nameless",
        basePath: "/nameless"
      }
    },
    {
      id: "DOKJASHEARTSCANLATION",
      className: "DokjasheartScanlation",
      title: "Dokjasheart scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "dokjasheart",
        basePath: "/dokjasheart"
      }
    },
    {
      id: "DORAGONNOTENSEI",
      className: "DORAGONNOTENSEI",
      title: "DORAGON NO TENSEI",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "dnt",
        basePath: "/dnt"
      }
    },
    {
      id: "DOUJINCLUB",
      className: "Doujinclub",
      title: "Doujinclub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
    ...
    },
    {
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "eljeksreal",
        basePath: "/eljeksreal"
      }
    },
    {
      id: "ELSCANSEMANAL",
      className: "ElScanSemanal",
      title: "El Scan Semanal",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "elscansemanal",
        basePath: "/elscansemanal"
      }
    },
    {
      id: "ERISGREYRATTRANSLATIONS",
      className: "ErisGreyratTranslations",
      title: "Eris Greyrat Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "erisgreyrattranslations",
        basePath: "/erisgreyrattranslations"
      }
    },
    {
      id: "FAFA",
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "fafa",
        basePath: "/fafa"
      }
    },
    {
      id: "FAXXTRADUCE",
      className: "FaxxTraduce",
      title: "FaxxTraduce",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "faxxtraduce",
        basePath: "/faxxtraduce"
      }
    },
    {
      id: "FSTAKESUKYAN",
      className: "FstakeSukyan",
      title: "Fstake Sukyan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "fstakesukyan",
        basePath: "/fstakesukyan"
      }
    },
    {
      id: "GLOWI",
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "glowi",
        basePath: "/glowi"
      }
    },
    {
      id: "HACHIMITSUTSUKI",
      className: "HachimitsuTsuki",
      title: "Hachimitsu Tsuki",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "shojopapers",
        basePath: "/shojopapers"
      }
    },
    {
      id: "HANGYAKUNOSCANLATION",
      className: "HangyakuNoScanlation",
      title: "Hangyaku no Scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "hangyakunoscanlation",
        basePath: "/hangyakunoscanlation"
      }
    },
    {
      id: "HARUSCAN",
    ...
    },
    {
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "joshscan",
        basePath: "/joshscan"
      }
    },
    {
      id: "KAISERZSCANLATION",
      className: "KaiserZScanlation",
      title: "KaiserZ Scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "kaizscan",
        basePath: "/kaizscan"
      }
    },
    {
      id: "KAIZENTRANSLATIONS",
      className: "KaizenTranslations",
      title: "Kaizen Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "kaizen",
        basePath: "/kaizen"
      }
    },
    {
      id: "KATSUDONMANGA",
      className: "KatsudonManga",
      title: "KatsudonManga",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "katsudonmanga",
        basePath: "/katsudonmanga"
      }
    },
    {
      id: "KAWARAGINOFANSUB",
      className: "KawaragiNoFansub",
      title: "Kawaragi No Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "kawaraginofansub",
        basePath: "/kawaraginofansub"
      }
    },
    {
      id: "KINGDOMDARKSCAN",
      className: "KingdomDarkScan",
      title: "Kingdom Dark Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "kingdomdarkscan",
        basePath: "/kingdomdarkscan"
      }
    },
    {
      id: "KIRISHIMAFANSUB",
      className: "KirishimaFansub",
      title: "Kirishima Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "kirishimafansub",
        basePath: "/kirishimafansub"
      }
    },
    {
      id: "KORONESUKI",
      className: "Koronesuki",
      title: "Koronesuki",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
    ...
    },
    {
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "krazytoon",
        basePath: "/krazytoon"
      }
    },
    {
      id: "KURENAIPROJECT",
      className: "KurenaiProject",
      title: "Kurenai Project",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "kurenaiproject",
        basePath: "/kurenaiproject"
      }
    },
    {
      id: "KUROTRANSLATIONS",
      className: "KuroTranslations",
      title: "KuroTranslations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "kurotranslations",
        basePath: "/kurotranslations"
      }
    },
    {
      id: "LABIBLIOTECADEODIN",
      className: "LaBibliotecaDeOdin",
      title: "La Biblioteca de Odin",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "labibliotecadeodin",
        basePath: "/labibliotecadeodin"
      }
    },
    {
      id: "LASPROFANADASSCAN",
      className: "LasProfanadasScan",
      title: "Las Profanadas Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "profanadasscan",
        basePath: "/profanadasscan"
      }
    },
    {
      id: "LASTEXILESCAN",
      className: "LastExileScan",
      title: "Last Exile Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "lastexilescan",
        basePath: "/lastexilescan"
      }
    },
    {
      id: "LIMBOSCANLATION",
      className: "LimboScanlation",
      title: "Limbo Scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "limbo",
        basePath: "/limbo"
      }
    },
    {
      id: "LOLIETAZURATRANSLATIONS",
      className: "LolietAzuraTranslations",
      title: "Loliet Azura Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "lolietazura",
        basePath: "/lolietazura"
      }
    },
    {
      id: "MICHUSCAN",
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "michuscan",
        basePath: "/michuscan"
      }
    },
    {
      id: "NEROTRANSLATIONS",
      className: "NeroTranslations",
      title: "NeroTranslations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "nerotranslate",
        basePath: "/nerotranslate"
      }
    },
    {
      id: "OKAMISCAN",
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "okami",
        basePath: "/okami"
      }
    },
    {
      id: "OMEGASCANLATION",
      className: "OmegaScanlation",
      title: "Omega Scanlation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "omegascanlation",
        basePath: "/omegascanlation"
      }
    },
    {
      id: "PHANTOMNOFANSUB",
      className: "PhantomNoFansub",
      title: "Phantom No Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "phantomnofansub",
        basePath: "/phantomnofansub"
      }
    },
    {
      id: "PINKGARDENTRANSLATION",
      className: "PinkGardenTranslation",
      title: "PinkGarden Translation",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "pinkgardentranslation",
        basePath: "/pinkgardentranslation"
      }
    },
    {
      id: "PUCHISCAN",
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "puchi",
        basePath: "/puchi"
      }
    },
    {
      id: "RAKUENTRANSLATIONS",
      className: "RakuenTranslations",
      title: "Rakuen Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "rakuen",
        basePath: "/rakuen"
      }
    },
    {
      id: "SACACHISPA",
      className: "SACACHISPA",
      title: "SACACHISPA",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "sacachispa",
        basePath: "/sacachispa"
      }
    },
    {
      id: "SAITOUTRANSLATIONS",
      className: "SaitouTranslations",
      title: "Saitou Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "saitoutranslations",
        basePath: "/saitoutranslations"
      }
    },
    {
      id: "SCANREVIEW",
      className: "ScanReview",
      title: "Scan Review",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
    ...
    },
    {
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "scanshub",
        basePath: "/scanshub"
      }
    },
    {
      id: "SCANSTARTS",
      className: "ScanStarts",
      title: "Scan Starts",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
    ...
    },
    {
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "scanz",
        basePath: "/scanz"
      }
    },
    {
      id: "SECTABLSCAN",
      className: "SectaBLScan",
      title: "Secta BL Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "sectablscan",
        basePath: "/sectablscan"
      }
    },
    {
      id: "SEINENDEN",
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "seinen-den",
        basePath: "/seinen-den"
      }
    },
    {
      id: "SEORCHOFLI",
      className: "SeOrChofli",
      title: "Se\xF1or Chofli",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "sirchofli",
        basePath: "/sirchofli"
      }
    },
    {
      id: "SENSHIMANGA",
      className: "SenshiManga",
      title: "Senshi Manga",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      ...
      overrides: {
        ...
        basePath: "/senshimanga"
      }
    },
    {
      id: "SHOSHINSHASCANS",
      className: "ShoShinShaScans",
      title: "ShoShinSha Scans",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "shoshinshascans",
        basePath: "/shoshinshascans"
      }
    },
    {
      id: "6IANFRANC9",
      className: "6ianfranc9",
      title: "6ianfranc9",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
    ...
    {
      id: "SOULCRAFTFANSUB",
      className: "SoulCraftFansub",
      title: "Soul Craft Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "soulcraft",
        basePath: "/soulcraft"
      }
    },
    {
      id: "SPACEFANSUB",
      className: "SpaceFansub",
      title: "Space Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "spacefansub",
        basePath: "/spacefansub"
      }
    },
    {
      id: "STRAYDOG608",
      className: "StrayDog608",
      title: "StrayDog608",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "straydog608",
        basePath: "/straydog608"
      }
    },
    {
      id: "SWEETICECREAM",
      className: "SweetIceCream",
      ...
      isNsfw: true,
      overrides: {
        organizationDomain: "sweeticecream",
        basePath: "/sweeticecream"
      }
    },
    {
      id: "TEMPLODEJENY",
      className: "TemploDeJeny",
      title: "Templo de Jeny",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "templodejeny",
        basePath: "/templodejeny"
      }
    },
    {
      id: "TEMPLODESCARLET",
      className: "TemploDeScarlet",
      title: "Templo de Scarlet",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "templodescarlet",
        basePath: "/templodescarlet"
      }
    },
    {
      id: "TERRASCANS",
      className: "TerraScans",
      title: "Terra Scans",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "terrascans",
        basePath: "/terrascans"
      }
    },
    {
      id: "THEBLUEBOX",
      className: "TheBlueBox",
      title: "TheBlueBox",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "thebluebox",
        basePath: "/thebluebox"
      }
    },
    {
      id: "THEGRIMORIODELAWITCH",
      className: "TheGrimorioDeLaWitch",
      title: "The grimorio de la witch",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "tgdlw",
        basePath: "/tgdlw"
      }
    },
    {
      ...
      isNsfw: false,
      overrides: {
        organizationDomain: "tamt",
        basePath: "/tamt"
      }
    },
    {
      id: "TRADUCCIONESMUYLENTAS",
      className: "TraduccionesMuyLentas",
      title: "Traducciones Muy Lentas",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "tmlentas",
        basePath: "/tmlentas"
      }
    },
    {
      id: "TRADUCCIONESPALTANOBU",
      className: "TraduccionesPaltaNobu",
      title: "Traducciones PaltaNobu",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "paltanobu",
        basePath: "/paltanobu"
      }
    },
    {
      id: "TSUKINOMUSUMESCAN",
      className: "TsukiNoMusumeScan",
      title: "Tsuki No Musume Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "tsukinomusumescan",
        basePath: "/tsukinomusumescan"
      }
    },
    {
      id: "TUERIZOTRADUCCIONES",
      className: "TuErizoTraducciones",
      title: "Tu Erizo Traducciones",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "tuerizotraducciones",
        basePath: "/tuerizotraducciones"
      }
    },
    {
      id: "VIRREINATODEUNKNOWNREALMFANSUB",
      className: "VirreinatoDeUnknownRealmFansub",
      title: "Virreinato de Unknown Realm Fansub",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "unknown-realm-fansub",
        basePath: "/unknown-realm-fansub"
      }
    },
    {
      id: "WAIFUSWITHSALSASCAN",
      className: "WaifusWithSalsaScan",
      title: "WaifusWithSalsa Scan",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "wws",
        basePath: "/wws"
      }
    },
    {
      id: "WEEPINGANGEL",
      className: "WeepingAngel",
      title: "Weeping Angel",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: true,
      overrides: {
        organizationDomain: "weepingangel",
        basePath: "/weepingangel"
      }
    },
    {
      id: "WHITEPEARLTRANSLATIONS",
      className: "WhitePearlTranslations",
      title: "White Pearl Translations",
      locale: "es",
      domain: "capibaratraductor.com",
      family: "LectorMoeParser",
      isNsfw: false,
      overrides: {
        organizationDomain: "whitepearl",
        basePath: "/whitepearl"
      }
    },
    {
      id: "COMICBORDER",
      className: "ComicBorder",
      title: "Comic Border",
      locale: "ja",
      domain: "comicborder.com",
      family: "GigaViewerParser",
      isNsfw: false,
      overrides: {}
    },
    {
    ...
    },
    {
      ...
      className: "TonariNoYoungJump",
      title: "Tonari no Young Jump",
      locale: "ja",
    ...
    {
      id: "MANGABANGCOMICS",
      className: "MangaBangComics",
      title: "MangaBang Comics",
      locale: "ja",
